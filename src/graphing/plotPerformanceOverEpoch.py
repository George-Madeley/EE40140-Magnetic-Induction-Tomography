...
def plotPerformanceOverEpoch(
  ...
  Args:
    material (Literal['copper', 'aluminium'], optional): The material to plot the performance for. Defaults to 'copper'.
    model (Literal['NN', 'GAN', 'VAE'], optional): The model to plot the performance for. Defaults to 'NN'.
    metric (Literal['BCE', 'BCELogits', 'MSE', 'MAE'], optional): The metric to plot. Defaults to 'BCE'.
    verbose (bool, optional): Whether to display the plot. Defaults to False.
    epoch_limit (int, optional): The maximum epoch to include in the plot. Defaults to 1000.
  """
  ...
  for f in files:
    ...
    plt.xlabel('Epoch')
    plt.ylabel(metric)
    # Add this line to use scientific notation on the y-axis
    ...
    plt.savefig(
        os.path.join(
            saveDir,
            f'{modelVersion} {modelId} - {metric}.png'))
...
def getColumns(
  ...
  modeColumns: List[str],
        mode: str) -> pd.DataFrame:
  """
  Extracts columns from the data based on the mode.

  Args:
    data (pd.DataFrame): The input data.
    columns (List[str]): All columns in the data.
    metaColumns (List[str]): Columns before the mode columns.
    modeColumns (List[str]): Columns specific to the mode.
    mode (str): The mode (e.g., 'train' or 'test').

  Returns:
    pd.DataFrame: A new dataframe with the extracted columns.
  """
  subDfs = []
  if len(modeColumns) // 4 > 1:
    ...
    for subNetworkId in subNetworkIds:
      ...
      subNetworkData = subNetworkData.rename(
        columns={c: formatMetricName(c) for c in subNetworkColumns})
    ...
    modeData['mode'] = mode
    modeData = modeData.rename(
      columns={c: formatMetricName(c) for c in modeColumns})
...
if __name__ == '__main__':
  models: List[str] = ['NN', 'CNN', 'GAN', 'DCGAN', 'VAE', 'ResNet', 'UNN']
  metrics: List[str] = ['BCE', 'BCELogits', 'MSE', 'MAE', 'SSIM']
  materials: List[str] = ['copper', 'aluminium']
