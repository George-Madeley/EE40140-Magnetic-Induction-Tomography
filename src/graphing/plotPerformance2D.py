...
def plotPerformance2D(
  ...
  features = data.columns[data.columns.get_loc(
    'numSamples') + 1:data.columns.get_loc('Accuracy')]
  ...
  top_features: List[Tuple[str, str, float]] = []
  for x_feature in features:
    for y_feature in features:
      if x_feature == y_feature:
        continue
      data_grouped = data.groupby([x_feature, y_feature]).agg(
  ...
  plt_accuracy.savefig(
      os.path.join(
          save_dir,
          f'{formatString(model)} - Accuracy.png'))
...
def graphPlot(
  ...
  data = data.groupby([x_feature, y_feature]).agg(
    {hue: ['min', 'max', 'mean']}).reset_index()
  ...
  plt.gca().set_xticklabels([formatString(label.get_text())
                             for label in plt.gca().get_xticklabels()])
  plt.gca().set_yticklabels([formatString(label.get_text())
