...
def plotShapeNum():
  # Read the CSV file
  data = pd.read_csv(os.path.join('data', 'data_samples.csv'))
  ...
  aluminium_samples = data[data['sample'] < 'H']

  # pop samples from aluminium samples that are '0'
  null_samples = aluminium_samples[aluminium_samples['sample'] == '0']
  null_samples_counts = null_samples.groupby('shape').size()

  aluminium_samples = aluminium_samples[aluminium_samples['sample'] != '0']
  aluminium_sample_counts = aluminium_samples.groupby('shape').size()

  # Plot the stacked bar chart
  plt.bar(null_samples_counts.index, null_samples_counts, color=colors[0], label='Null Samples')
  plt.bar(aluminium_sample_counts.index, aluminium_sample_counts, color=colors[1], label='Aluminium Samples')
  plt.bar(copper_sample_counts.index, copper_sample_counts, bottom=aluminium_sample_counts, color=colors[2], label='Copper Samples')
