...
def plotPerformanceByArea(
  ...
  directory = os.path.join('results', 'generation', 'per image')
  files = os.listdir(directory)
  files = [f for f in files if f.endswith('.csv')]
  files = [os.path.join(directory, f) for f in files]
  df_performance = pd.concat([pd.read_csv(f) for f in files])

  data_path = os.path.join('data', f'data_samples_{material}.csv')
  ...
  for label, group in groups:
    ...
    cbar.set_label('Probability of Pixel Occupancy')
    plt.axis('off')
    ...
    save_path = os.path.join(save_dir, f'{label} - {metric} - {height}x{width}.png')
